...
function markCellCrossed(cell) {
  const currentCrossCount = Number(cell.dataset.crossCount || "0") + 1;
  cell.dataset.crossCount = String(currentCrossCount);
  if (currentCrossCount >= 2) {
    cell.classList.add("crossed-twice-cell");
  }
}
...
async function animateBacktracking() {
  ...
  for (let idx = 0; idx < state.path.length && !state.cancelRequested; idx += 1) {
    ...
    const movedDiagonally =
      nextPoint && nextPoint.i === point.i - 1 && nextPoint.j === point.j - 1;
    const isFinalBoundaryMatch =
      !nextPoint &&
      state.s1[point.i - 1] === state.s2[point.j - 1] &&
      state.table[point.i][point.j] === state.table[point.i - 1][point.j - 1] + 1;

    if (movedDiagonally || isFinalBoundaryMatch) {
